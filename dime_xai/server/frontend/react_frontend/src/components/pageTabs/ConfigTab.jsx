      ...
      .then(function (response) {
        ...
        if (response.data.status !== undefined) {
          if (response.data.status === "success") {
            ...
            this.setState({
              snackbarIsOpen: true,
              snackbarMessage: "No models found. Double check the models path in Configs",
              snackbarType: "error",
            })
    ...
    return (
      <>
        <Box
          ...
          <Box className="row p-3 justify-content-center">
            <Box className="alert app-alert mb-4" role="alert">
              <p className="white-to-black-ease">DIME configurations can be altered if required. Use either
                <kbd className="fs-6 kbd material-matt-black">Restore Previous</kbd> to set the configs to the
                previous
                version or <kbd className="fs-6 kbd material-matt-black">Reset</kbd> to reset the
                configurations to the initial version.
              </p>
            </Box>
            <Box sx={{ marginTop: 5 }}>
              <ConfigForm
        ...
        <Snackbar 
          ...
          <Alert 
            onClose={this.handleClose} 
            severity={this.state.snackbarType} sx={{ width: '100%' }}>
